...
**Description:**
Manages local rules (.cortex/rules/) for project-specific guidelines.
...
Phase 8 manages standardized project structure under `.cortex/`.
...
**Description:**
Manages standardized project structure including memory bank files, rules, and plans.
...
- **pre_commit_tools** – Phase A/B execution helpers; agent-facing zero-arg entrypoints are `run_quality_gate`, `run_docs_gate`, and `autofix` (format, type_check, quality, tests, docs sync).
