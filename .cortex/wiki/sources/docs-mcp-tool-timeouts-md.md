...
3. **Client-side behavior**  
   The client may run ListOfferings, GetInstructions, ListToolsRaw (and similar) when opening or refreshing the MCP connection. That is client-side and not controlled by Cortex.
...
- `project_root_resolver: list_roots() took X.XXs (client round-trip)` — time waiting for the client to respond to roots/list. If this is ~28s, the bottleneck is the client responding to the roots request.
...
Check the MCP server’s stderr (or the client's “Output” / MCP logs) for these lines after the first tool call.
...
- **When it helps**: Use HTTP/SSE or Streamable HTTP when you run Cortex as a long-lived server and connect from a client via URL instead of a shell command. Same tools and resources; only the transport and concurrency behavior change.
...
To get **concurrent MCP request handling** (e.g. ReadResource while a long tool runs) without running a separate server or changing the client's “on/off” flow, use the **bridge**:

- **Command**: Run `python -m cortex.bridge` (or the `cortex-bridge` script) as the client's MCP server command instead of `cortex.main`. The bridge starts Cortex with Streamable HTTP on a fixed port and proxies between the client (stdio) and Cortex (HTTP).
...
- **Result**: One switch in the client; Cortex runs with Streamable HTTP and can handle multiple requests concurrently, reducing resource read timeouts and "unknown message ID" issues.
...
- **Optional auth**: The MCP SDK supports optional auth (e.g. query token for SSE URL). See SDK and client docs for URL-based auth if you expose the server beyond localhost.
