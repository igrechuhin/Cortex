...
Cortex is structured as an MCP (Model Context Protocol) server that exposes **12 tools** and **6 static resources** on the live surface (plus setup prompts), while managing structured documentation (Memory Bank files). The system is built with a modular, layered architecture designed for:
...
┌─────────────────────────────────────────────────────────────┐
│                    MCP Server (FastMCP)                     │
│           12 tools + 6 resources (published MCP surface)    │
└────────────┬────────────────────────────────────────────────┘
             │
             ▼
┌────────────────────────────────────────────────────────────┐
...
|-----------|----------|-------------|
| **stdio** | Default, most IDE-integrated clients | JSON-RPC over stdin/stdout; one process per client session |
...
- **prompts/** – Prompt templates (e.g. commit, do, analyze, plan); registered with the MCP server for the client IDE.
...
**Submodule pattern**: Projects add Synapse via `git submodule add <url> .cortex/synapse/`. The initialize and setup_synapse prompts configure the submodule; `synapse(operation="sync")` (or equivalent) keeps it updated. Subagent prompt files are synced from `.cortex/synapse/claude-agents/` to `.claude/agents/` for Claude Code discovery.
