...
- When resolving Cortex, Synapse, or repo-root paths in Python tools, use the shared path resolver (`cortex.core.path_resolver` and related helpers) instead of hardcoded filesystem strings. Keep session-goal marker files such as `session-goal.md` under `.cortex/.session/` rather than directly under `.cortex/`.
