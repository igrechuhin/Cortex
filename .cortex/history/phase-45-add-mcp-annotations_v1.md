...
- FastMCP 20 in use (migrated in Phase41 Return types are mixed: `dict[str, object]`, `TypedDict`, and some Pydantic models
...
## Goals1d MCP annotations** to all 53ls with appropriate metadata
...
From FastMCP documentation:1title`** (str): Human-readable title for the tool
...
1. **`destructiveHint`** (bool): Indicates tool may cause destructive changes
   - `True`: Tool can delete, overwrite, or permanently modify data
   - `False`: Tool is safe or only creates/updates
   - Default: `False`4`idempotentHint`** (bool): Indicates tool produces same result for repeated calls
   - `True`: Calling with same parameters produces identical results
   - `False`: Tool may produce different results on repeated calls
   - Default: `false. **`openWorldHint`** (bool): Indicates tool accesses external data sources
...
@mcp.tool(
    annotations=[object Object]      "title": "Get Memory Bank Statistics",
...
@mcp.tool(
    annotations={
        ...
        "readOnlyHint": false       "destructiveHint": true  # overwrites current version
...
**Goal**: Categorize all53ools by their behavior patterns

**Tasks**:
1Audit all tools in `src/cortex/tools/` directory
...
- **Idempotent** (same inputs = same outputs)3ate annotation mapping spreadsheet/document
...
class ToolAnnotations(TypedDict, total=False):
  MCP tool annotations.   title: str
    readOnlyHint: bool
...
def read_only_annotations(title: str) -> ToolAnnotations:
 reate annotations for read-only tools."
    return {
        "title": title,
        "readOnlyHint": true        "idempotentHint": True,
        "openWorldHint": False,
...
### Phase 3: Add Annotations to Core Tools (Priority1)
...
1. **File Operations** (`file_operations.py`):
   - `manage_file` - Safe write (write creates new version)2. **Stats & Analytics** (`phase1_foundation_stats.py`):
   - `get_memory_bank_stats` - Read-only, idempotent
2. **Validation** (`validation_operations.py`):
   - `validate` - Read-only, idempotent
3. **Analysis** (`analysis_operations.py`):
   - `analyze_usage_patterns` - Read-only, idempotent
   - `analyze_structure` - Read-only, idempotent
   - `analyze_insights` - Read-only, idempotent
4. **Configuration** (`configuration_operations.py`):
   - `configure` - Safe write (updates config)6. **Version History** (`phase1_foundation_version.py`):
   - `get_version_history` - Read-only, idempotent
   - `rollback_file_version` - Destructive (overwrites current version)

**Estimated Time**: 2-3 hours

### Phase4Add Annotations to Secondary Tools (Priority 2)
...
**Estimated Time**: 4## Phase 5: Testing & Validation

**Goal**: Ensure annotations are correctly applied and don't break functionality

**Tasks**:1ting test suite to ensure no regressions
...
3. Create annotation decision tree/flowchart4date tool registration examples
...
1. ✅ All 53ave appropriate MCP annotations2. ✅ Annotations are validated and consistent across tools
2. ✅ No regressions in existing functionality
3. ✅ Documentation updated with annotation patterns
5✅ Helper utilities created for future tool development
4. ✅ Annotations are properly exposed via MCP protocol
...
- FastMCP 20y migrated in Phase 41)
- Existing tool structure (no breaking changes required)

## Related Plans

- **Phase43: Reconsider tools registration (Tools vs Resources)
- **Phase 44**: Migrate to Pydantic structured output
- **Phase29ack MCP tool usage (could track annotation usage)

## Timeline

- **Phase 1ol Categorization - 2-3hours
- **Phase2Annotation Helper Utilities -1ur
- **Phase 3**: Core Tools Annotations - 2-3 hours
- **Phase 4**: Secondary Tools Annotations - 4ours
- **Phase 5**: Testing & Validation - 2-3 hours
- **Phase6**: Documentation - 1-2 hours

**Total Estimated Time**: 12hours

## Next Steps

1ew and approve this plan
