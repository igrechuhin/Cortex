...
- [Phase 23: Fix CI Failure After Validation Refactor](../plans/phase-23-fix-ci-failure-validation-refactor.md) - ASAP (PLANNING) - Investigate and fix GitHub Actions CI failure from commit `612e` (validation refactor) - Quality check failed with exit code 1, preventing commit from passing CI - Impact: Unblock CI pipeline, ensure validation refactor doesn't break CI - Target completion: 2026-01-13
...
- [Phase 20: Code Review Fixes](../plans/phase-20-code-review-fixes.md) - ASAP (IN PROGRESS - Steps 1-3.1 complete) - Address all critical issues from comprehensive code review (2026-01-15) Steps 1 complete (test import error verified fixed, 15 type errors fixed), ✅ Step 3.1 complete (validation_operations.py split: 1063 lines, 60% reduction), remaining: fix 9 more file size violations (Step 3.2-3.10), fix security vulnerabilities (command injection, XSS, ReDoS) - Impact: Improve code quality from 8.7/10 to 90.5+/10, achieve rules compliance - Target completion: 2026-01-20
- [Phase 19: Fix MCP Server Crash](../plans/phase-19-mcp-server-crash.md) - ASAP (PLANNING) - Fix MCP server crash caused by `BrokenResourceError` in `stdio_server` TaskGroup - Server crashes when client disconnects or cancels requests, causing unhandled `ExceptionGroup` - Impact: Server instability, poor user experience - Target completion: 2026-01-16
- ✅ **Phase 17: Validate Roadmap Sync Command** - COMPLETE (2026-01-14) - Implemented `validate-roadmap-sync` command and MCP tool integration - Added `roadmap_sync` check_type to `validate()` MCP tool - Created `.cortex/synapse/prompts/validate-roadmap-sync.md` command file - Added comprehensive unit tests (18 tests, all passing) - Roadmap/codebase synchronization is now enforced in commit workflow Step 10
...
- [Phase 23: Fix CI Failure After Validation Refactor](../plans/phase-23-fix-ci-failure-validation-refactor.md) - PLANNING (2026-01-16) - Investigate and fix GitHub Actions CI failure from commit `612e` (validation refactor) - Quality check failed with exit code 1, preventing commit from passing CI - Root cause: Likely missing files, import errors, or quality violations from refactoring - Target completion: 2026-01-13
...
- [Phase 19: Fix MCP Server Crash](../plans/phase-19-mcp-server-crash.md) - PLANNING (2026-01-14) - Fix MCP server crash caused by `BrokenResourceError` in `stdio_server` TaskGroup - Server crashes when client disconnects or cancels requests, causing unhandled `ExceptionGroup` - Impact: Server instability, poor user experience - Target completion: 2026-01-16
