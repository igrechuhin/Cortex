...
- **Content-Preserving WAL Pattern (AS-OF reconstruction)** - `src/cortex/memory/wal.py` records memory-bank mutations; `src/cortex/memory/wal_content.py` adds the content layer. Each `WALEntry` carries `reverse_delta` (zlib+base64 of the *prior* content), `delta_codec` (`none` legacy | `zlib-b64` | `pruned`), and `step_number` (highest experience-store step for the writing session, resolved best-effort by `wal_current_step_number()`). Only files under `.cortex/memory-bank/` retain content; everything else gets the step number alone. `wal_as_of(project_root, file, step_number)` returns a typed `WalAsOfResult` by walking to the first recorded write *after* that step and decoding what it overwrote, hash-verified against `content_hash_before` (mismatch raises); when no later write exists the on-disk content is returned with `verified` reflecting whether it still matches `content_hash_after`. Exposed to the analyze pipeline as `memory_wal(operation="as_of", file=..., step_number=...)` - no direct WAL file access. Growth is bounded on every append by `wal_compact_log_bytes()`: under budget it is a no-op byte comparison, over budget it downgrades the oldest entries to `pruned` (payload dropped, audit fields kept) and finally drops the oldest lines. Legacy entries (no `step_number`) stay readable and are skipped during reconstruction.
