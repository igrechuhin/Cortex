...
- **Fix analyze pipeline phase-allowlist and subagent tool-grant gaps** - COMPLETE. Extended pipeline_handoff phase allowlist with context/session/tools; granted ReadMcpResourceTool/Bash/Write to the four analyze-* subagents via canonical Synapse source; added phase-validation tests. Quality gate passed (7273 tests, 91.09% coverage).
- **Prompt-Cache Payload Stability for Cached MCP Resources** - COMPLETE. Fixed genuine payload non-determinism (PYTHONHASHSEED-dependent set ordering in ContextDetector; equal-mtime glob-order ties in recent artifacts/sources context builders) so cortex://rules and cortex://context produce byte-identical output across reads of unchanged state, restoring the value of the already-wired Anthropic cache_control hints. Added audit_cache_payload_stability() quality-gate check to prevent regression. 13 new tests, 91.11% coverage, quality gate green.
- <!-- memory_type: problem -->
- **Tool-Invocation Telemetry Log to Strengthen Skill-Crystallization Signal** - COMPLETE. Added session-scoped redacted tool-invocation telemetry log (WAL-backed) with a memory_wal read accessor consumed by analyze-tools/analyze-session; fixed a reopened review gap where cancelled tool calls were mislabeled as successes.
- **Embedding-Based Relevance Scoring for Context Load/Compaction Gating** - COMPLETE. Added relevance_ranking.py (cosine+BM25 blend reusing experience-store embedding primitives) wired into l0_identity._truncate_to_budget and l2_on_demand._truncate_paragraphs behind CORTEX_RELEVANCE_RANKING_ENABLED env flag (default disabled, fails open to positional truncation on any error). 25 new tests, 91.15% coverage, quality gate green.
- **Git-Backed Sandboxed Self-Modification Proposal Tool** - COMPLETE. Added propose_framework_optimization MCP tool: isolated git worktree (detached), path-allowlisted apply (.cortex/synapse/ and .cortex/rules/ only, traversal-rejecting), schema/frontmatter/size self-test, difflib-based reviewable diff output, guaranteed try/finally teardown, zero push/PR code paths. 44 tests, 91.18% coverage, quality gate green.
- **Task-Level Stuck-Loop Constraints Monitor Beyond MCP Circuit Breaker** - COMPLETE. Added no-progress detector for fix-tests/fix-quality/implement-code subagents (AttemptRecord + detect_no_progress in src/cortex/core/no_progress_monitor.py), distinct from the existing MCP-transport circuit breaker; subagent prompts and shared-conventions.md updated; 23 new tests, 100% coverage on new code, no regressions (7402/7402 passing, 91.19% repo coverage).
- <!-- memory_type: status -->
- **Support xcodebuild -skip-testing: via .cortex/config/swift_test.json** - COMPLETE. Added SwiftTestConfig (src/cortex/config/swift_test_config.py) mirroring the swift_coverage.json pattern; wired into SwiftXcodebuildMixin._xcodebuild_test_phase so each configured skip_testing identifier becomes an xcodebuild -skip-testing: flag ahead of the test-without-building action, fixing quality-gate failures on infra a project's own CLAUDE.md documents as intentionally excluded. New tests: tests/unit/test_swift_test_config.py, tests/unit/test_swift_xcodebuild_mixin.py. Phase A: scope=source, coverage=N/A.
**Tool-Invocation Telemetry Log to Strengthen Skill-Crystallization Signal** - PARTIAL (reopened by review). Implemented all 7 plan steps: ToolInvocationEntry/ToolInvocationLog append-only JSONL telemetry (src/cortex/memory/wal.py), redaction/append helpers (src/cortex/memory/wal_hooks.py), hook into the existing mcp_tool_wrapper dispatch interception point (src/cortex/core/mcp_tool_telemetry.py, src/cortex/core/mcp_stability.py), memory_wal(operation="tool_invocations") read accessor (src/cortex/tools/memory/wal_tool.py), and analyze-tools.md prompt updates (3 mirrors, verified in sync). 20 tests added, ~91% coverage, run_quality_gate() green. Review gate found one correctness gap: cancelled MCP tool calls are recorded as success (WalStatus.OK) instead of error/CancelledError in the new telemetry log, inconsistent with the existing UsageTracker path it was modeled on. Plan left PENDING with a Review Follow-Up Gaps entry for the next /cortex/do pass.
...
- <!-- memory_type: status -->
**Content-Preserving WAL for AS-OF Reconstruction - gate check** - Checked the plan's documented execution gate ("execute only after analyze-experience-graph-queries proves valuable") via direct `pipeline_handoff` graph queries: `preference_pairs`/`repeated_failures` returned zero coverage even with real Do Loop work present in the graph; `fitness_by_task_type` showed 18 real fitness-scored "commit" samples. Gate condition not yet satisfied - plan left PENDING, not archived (`.cortex/plans/content-preserving-wal-as-of.md`).
**Pipeline Handoff op_init/op_clear Idempotency Fix** - COMPLETE. Closed the remaining root cause from the 2026-07-21 pipeline_handoff phase-state-loss investigation: `op_clear` was wiping ALL phases instead of respecting the `phase` argument passed to it, compounding the previously-fixed `op_init` non-idempotency bug. Both fixed in `pipeline_handoff_io.py`/`pipeline_handoff.py`; investigation plan closed and archived at `.cortex/plans/archive/Investigations/investigate-pipeline-handoff-phase-state-loss-during-long-running-subagent-calls.md`.
