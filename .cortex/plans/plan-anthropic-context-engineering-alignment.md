...
**Status:** Complete. Rubric added (see [Tool Description Altitude Rubric](../../docs/guides/tool-description-altitude-rubric.md)). Twenty-two batches applied. Twenty-second batch: `get_dependency_graph` (Example error added); `get_memory_bank_stats` (Args response_format + Example error); `list_session_scripts` (Args + Example success); `promote_session_script` (Args + Example success and error). All catalogued MCP tools have been audited and score ≥ 4 on the altitude rubric. 20+ tools have embedded examples. **Acceptance criteria met:** All tools ≥ 4/5; 20+ with embedded examples.
...
**Status:** In progress. Response token fields added to usage tracking; wrapper-level counting and analysis still pending.
