...
**Status:** In progress. Rubric added (see [Tool Description Altitude Rubric](../../docs/guides/tool-description-altitude-rubric.md)). Pilot: 5 tools improved with EXAMPLES (list_plans, get_plan, add_roadmap_entry, append_progress_entry, append_active_context_entry). Second batch: 5 more (remove_roadmap_entry, remove_roadmap_section, complete_plan, compact_session, register_plan_in_roadmap). Third batch: 5 more (create_plan, run_preflight_checks, run_docs_and_memory_bank_sync, query_memory_bank, query_usage) — 15 tools total with embedded examples. Fourth batch: audited altitude for core context/rules tools (`load_context`, `manage_file`, `query_memory_bank`, `query_usage`) and confirmed they meet the rubric. Fifth batch: `execute_pre_commit_checks` docstring expanded to full altitude; `get_structure_info` doc corrected (no project_root; root resolved internally). Sixth batch: `check_structure_health` doc fixed (removed project_root from Args; root resolved internally); full altitude added to `run_tool_evaluation`, `analyze_error_patterns`, `benchmark_model`; `get_relevance_scores` parameter guidance (include_sections) added. Seventh batch: `read_cache_json` and `write_cache_json` brought to full altitude (EXAMPLES, RETURNS, Args); EXAMPLES added to `claim_task_lock`, `release_task_lock`, `list_active_tasks`, `check_task_available_lock`. Eighth batch: `analyze_health_check` Args added (analysis_type, similarity_threshold, include_dependencies, validate_quality); `session_scripts` brought to full altitude (USE WHEN, EXAMPLES, RETURNS, Args for all operations). Ninth batch: `summarize_content` Args added; `suggest_tool_improvements` and `analyze_session_scripts` Args added; `compact_session` RETURNS line added. Tenth batch: `fix_markdown_lint` Args added (include_untracked_markdown, dry_run, check_all_files); `quick_start` Args and EXAMPLES expanded; `quality_check` EXAMPLES added; `safe_manage_file` brought to full altitude (EXAMPLES, RETURNS, Args for all parameters). Eleventh batch: `get_synapse` brought to full altitude (USE WHEN, EXAMPLES, RETURNS, Args for all params); `fix_roadmap_corruption` Args added (dry_run); `update_synapse` RETURNS and EXAMPLES clarified. Twelfth batch: `cleanup_metadata_index` Args added (dry_run) and RETURNS key fields documented. Thirteenth batch: embedded EXAMPLES added to `list_available_tools`, `search_tools`, `fix_quality_issues`, `session_start`, `suggest_workflow` (Example JSON in docstrings). Fourteenth batch: `think` and `sequentialthinking` (Args + Example JSON); `capture_session_script` (Args + Example); `list_plans` and `get_plan` (Args + Example JSON). Full audit of remaining 46+ tools pending.
