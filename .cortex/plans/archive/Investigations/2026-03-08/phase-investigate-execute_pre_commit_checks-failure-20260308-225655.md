...
- Investigation confirmed that the `Another long-running tool is in progress` error is emitted only when the long-running semaphore is legitimately held by another call.
- New long-running semaphore helpers, wait/hold configuration, and serialization tests in `tests/unit/test_mcp_stability_timeouts.py` all pass.
- Roadmap sync behavior and exclusions tests (`tests/unit/test_roadmap_sync.py`, `tests/unit/test_roadmap_sync_exclusions.py`) pass, confirming no regressions from the semaphore changes.
- With no concurrent long-running tools active, `execute_pre_commit_checks` is expected to run normally; the concurrency error now reflects the intended protection rather than a protocol bug.
