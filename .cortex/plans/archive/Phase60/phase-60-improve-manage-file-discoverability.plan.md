...
Improve the discoverability and error experience of the `manage_file` MCP tool and closely related high-traffic tools (such as `rules`) so that:

- Callers (LLMs and humans) can easily discover required parameters (for example, `file_name` and `operation` for `manage_file`, and `operation` for `rules`).
- Missing/invalid parameter errors are surfaced as clear, actionable, structured responses instead of opaque Pydantic validation traces.
- Higher-level workflows (e.g., commit pipeline via `execute_pre_commit_checks` Step 0 `fix_errors`) can handle MCP tool misuse gracefully without obscure schema/validation failures.
...
- In a subsequent session, the `rules` MCP tool was also invoked without its required `operation` field, producing a similar Pydantic `Field required` validation error.
- These failures surfaced as generic validation errors, with limited guidance on how to correct the calls.
- Cortex strongly encourages using MCP tools (`manage_file`, `rules`, `get_memory_bank_stats`, etc.) for memory bank and rules operations, so their ergonomics and discoverability significantly affect developer and LLM experience.
- Existing roadmap/plans already cover broader MCP tool description improvements and validation error UX; this plan focuses on `manage_file` and other MCP tools with critical required parameters as high-impact, frequently used tools.
...
### Phase 60.3: Generalization to Other Tools (Starting with `rules`)

1. **Cross-Tool Survey**
   - Identify other high-traffic MCP tools with required parameters that may suffer from similar discoverability issues.
   - Treat the `rules` tool as the first concrete generalization target, since it already produced a `rulesArguments.operation` missing-field error in real sessions.
   - Additional candidates may include memory bank tools (`get_memory_bank_stats`, etc.), validation tools (`validate`, `check_structure_health`), optimization tools, etc.

2. **Pattern Extraction**
   - If recurring patterns of missing-argument errors are found:
     - Define a shared internal helper for building consistent validation error responses across MCP tools with required parameters.
     - Consider a simple `get_tool_schema(tool_name)` helper or documentation link pattern to encourage schema introspection from agents.
   - For `rules`, design error responses that clearly state that `operation` is required, list valid operations, and provide examples of correct usage.

3. **Scope Control**
   - Extend the `manage_file` improvements to a small set of critical tools first (`rules` and any other tools that show real-world validation failures).
   - Avoid over-engineering a full discovery API unless clear demand exists.

**Exit criteria (Phase 60.3):**

- Decision documented on how far to generalize beyond `manage_file`.
- `rules` updated and validated with the same friendly validation and discoverability pattern.
- At least one additional high-impact MCP tool updated if recurring issues are identified.
