...
## Related plans (compound alignment)

These plans reduce recurring friction and make the next session easier; they are part of compound-engineering alignment. Implementation details live in the plans below; this plan adds narrative, checklist, and doc framing.

- **session-optimization-implement-prompt-memory-bank** (archive) — memory bank writes via `manage_file` only at step start.
- **session-optimization-commit-pipeline-improvements** (archive and active `.cortex/plans/`) — commit pipeline phases, preflight, quality gate.
- **session-optimization-implement-load-context-and-rules-fallback** (archive) — load context at step start, task-type token budgets, rules fallback.
- **session-optimization-roadmap-full-content-enforcement** (archive) — roadmap single-line edits via read-then-write; safe update tools.
- **session-optimization-load-context-on-problem-fix-path** (plans) — require load context when fixing so agents follow project rules.
- **session-optimization-roadmap-completed-section-cleanup** (plans) — cleanup legacy completed sections; single-block edit pattern.
