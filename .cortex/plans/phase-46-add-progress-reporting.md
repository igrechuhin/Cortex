...
Add progress reporting to long-running MCP tools using FastMCP's `Context.report_progress()` capability. This will improve user experience by providing real-time feedback during operations that take longer than 10 seconds.

## Context

### Current State

- **FastMCP Version**: 2.14.3 (migrated in Phase 41)
- **Total Tools**: 53+ MCP tools across 21+ modules
- **Timeout Categories**:
  - `MCP_TOOL_TIMEOUT_FAST`: 60s (simple operations)
  - `MCP_TOOL_TIMEOUT_MEDIUM`: 120s (validation, parsing)
  - `MCP_TOOL_TIMEOUT_COMPLEX`: 300s (context optimization, progressive loading)
  - `MCP_TOOL_TIMEOUT_VERY_COMPLEX`: 600s (refactoring, comprehensive analysis)
...
- **Access Methods**:
  - `CurrentContext()` - Dependency injection (preferred for FastMCP 2.14+)
  - `get_context()` - Function to retrieve context from nested calls
...
1. **Identify Candidate Tools**: Tools with timeouts ≥ 120s (MEDIUM, COMPLEX, VERY_COMPLEX)
2. **Set Progress Threshold**: Report progress for tools running longer than 10 seconds
3. **Implement Progress Reporting**: Add `ctx.report_progress()` calls at key execution stages
4. **Two-Tier Approach**:
...
5. **Testing**: Verify progress reporting works correctly with FastMCP clients
...
- 10 seconds is long enough to warrant feedback but short enough to be useful
- Aligns with timeout categories (MEDIUM=120s, COMPLEX=300s, VERY_COMPLEX=600s)

### Tool Categorization

**Tools Requiring Progress Reporting** (timeout ≥ 120s):

1. **COMPLEX Tools (300s)**:
        ...
        try:
            ...
            if ctx:
                ...
                await ctx.report_progress(
                    ...
                    total=100,
                    message=f"{self.tool_name}: {message}" if message else None
                )
        except Exception:
...
- Track execution time and report progress at intervals (every 10 seconds)
- Report progress automatically for tools with timeouts ≥ 120s
- Provide generic progress messages ("Processing...", "50% complete", etc.)
...
def mcp_tool_wrapper(
    ...
    Args:
        ...
        enable_progress: If True, enable progress reporting. If None, auto-enable
            for timeouts >= 120 seconds.
    """
...
1. `optimize_context_impl` - 4 stages
2. `load_progressive_context_impl` - 3-5 stages
3. `summarize_content_impl` - 3 stages
4. `get_relevance_scores_impl` - 3 stages
5. `resolve_transclusions_impl` - 2-3 stages
6. `validate_links_impl` (all files) - 2-3 stages
7. `suggest_refactoring_impl` - 4 stages
8. `apply_refactoring_impl` - 5-6 stages
...
### Decision 1: Automatic vs. Manual Progress Reporting
...
- Aligns with timeout categories (tools with 120s+ timeouts)

### Decision 3: Progress Calculation Method
...
1. ✅ All tools with timeouts ≥ 120s report progress automatically
2. ✅ Complex tools (COMPLEX, VERY_COMPLEX) have detailed stage-based progress
3. ✅ Progress reports appear in MCP clients (Claude Desktop, Cursor)
4. ✅ No performance degradation from progress reporting
5. ✅ Tests pass with 90%+ coverage
6. ✅ Progress reporting works in both MCP and test contexts
...
### Risk 3: Progress Reports Not Visible in Clients

**Mitigation**:

- Test with multiple MCP clients (Claude Desktop, Cursor)
- Verify FastMCP 2.14.3 progress reporting support
...
- **FastMCP 2.14.3**: Already migrated (Phase 41)
- **CurrentContext()**: Available in FastMCP 2.14+
- **MCP Client Support**: Clients must support progress reporting (verify)
