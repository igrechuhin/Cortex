...
`run_quality_gate()` → `_run_quality_gate_inner()`:
...
Currently `_run_quality_gate_inner` calls `clear_all_cached_results(root)` unconditionally before
...
- In `_run_quality_gate_inner`: gate the `clear_all_cached_results` call behind `if force_fresh:`
...
response-trimming step in `_run_quality_gate_inner`:
...
- File: `src/cortex/tools/execution/pre_commit_zero_arg_tools.py`
- Add `_trim_passing_result(result: ModelDict) -> ModelDict` helper
...
- [ ] `force_fresh=False` prevents cache clear; `force_fresh=True` still clears (Step 1)
- [ ] Passing run quality gate response is < 800 tokens (manual verification) (Step 2)
- [ ] Failing run quality gate response still includes full check details (Step 2)
- [ ] `agent_log` suppressed on passing runs (Step 3)
- [ ] `PipelineDirtyTracker` activated after successful quality gate (Step 4)
- [ ] Poll interval adaptive: 1s first 30s, 3s after (Step 5)
- [ ] Skip events visible in structured log (Step 6)
