...
- **SwiftFormat version pinning**: CI must pin `SWIFTFORMAT_VERSION` in `.github/workflows/quality.yml` to the same version installed locally (`swiftformat --version`). CI uses a binary downloaded from GitHub releases (not `brew upgrade`). When upgrading: bump `SWIFTFORMAT_VERSION` in CI, run `brew upgrade swiftformat` locally, run `swiftformat .` to apply any new formatting, then commit all three changes together. Version skew is the most common cause of "green locally, red CI" for Swift formatting.
- **swiftformat `--maxwidth` semantics**: lines of exactly `--maxwidth N` characters trigger the `wrap` rule in newer swiftformat versions (i.e., the limit is exclusive — lines must be `< N` chars). Keep all function names and lines strictly under the configured maxwidth.
