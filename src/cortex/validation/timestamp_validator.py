...
def _process_line_timestamps(
    line: str, line_num: int
) -> tuple[int, int, list[TimestampViolationModel]]:
...
def scan_timestamps(content: str) -> TimestampScanResult:
    ...
    Returns:
        TimestampScanResult with validation results
    """
    valid_count = invalid_format_count = 0
    violations: list[TimestampViolationModel] = []
...
) -> tuple[FileTimestampResultModel, int, int, int]:
...
async def validate_timestamps_all_files(
    ...
    results: dict[str, FileTimestampResultModel] = {}
...
def _process_all_files_timestamps(
    files_content: dict[str, str], results: dict[str, FileTimestampResultModel]
) -> tuple[int, int, int]:
...
def _build_timestamps_result(
    totals: tuple[int, int, int], results: dict[str, FileTimestampResultModel]
) -> AllFilesTimestampResult:
