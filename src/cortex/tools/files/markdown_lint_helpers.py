...
__all__ = [
    ...
    "build_markdownlint_error_result",
...
def result_success(result: GitCommandResult) -> bool:
    return result.success


def result_stdout(result: GitCommandResult) -> str:
    return result.stdout


def result_stderr(result: GitCommandResult) -> str:
    return result.stderr


def result_returncode(result: GitCommandResult) -> int | None:
    return result.returncode


def result_error(result: GitCommandResult) -> str | None:
    return result.error


def parse_markdownlint_errors(stderr: str) -> list[str]:
...
def parse_markdownlint_output(stdout: str) -> list[str]:
...
def parse_markdownlint_lines_by_file(output: str) -> dict[str, list[str]]:
...
def build_markdownlint_error_result(
    rel: str,
    lines: list[str],
    result: GitCommandResult,
) -> FileResult:
    """Return FileResult for a failed markdownlint run on one file."""
    error_msg = result_error(result) or "; ".join(lines[:3]) if lines else ""
    return build_error_result(
        rel,
        lines,
        result_returncode(result),
...
def _one_file_markdownlint_result(
    ...
    if rel in by_file:
        return build_markdownlint_error_result(rel, by_file.get(rel, []), result)
    return _build_markdownlint_success_result(rel, [], [], False, single_file, dry_run)


def build_markdownlint_batch_results(
    rel_strs: list[str],
    result: GitCommandResult,
    by_file: dict[str, list[str]],
    raw_lines: list[str],
...
def chunk_paths(files: list[Path], size: int) -> list[list[Path]]:
