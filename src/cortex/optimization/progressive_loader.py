    ...
    async def _process_file_for_dependencies(
        ...
        try:
            ...
            if cumulative_tokens + tokens <= token_budget:
                ...
                metadata = await self.metadata_index.get_file_metadata(file_name) or {}
...
def _extract_relevance_scores_from_metadata(
    ...
    relevance_scores_raw: object = metadata.get("relevance_scores", {})
    if isinstance(relevance_scores_raw, dict):
        relevance_scores_dict = cast(dict[str, float | int], relevance_scores_raw)
        for key, value_raw in relevance_scores_dict.items():
            relevance_scores[key] = float(value_raw)
...
def _build_loaded_content_from_optimization(
    ...
    # Access selected_files and excluded_files from optimization_result
    selected_files = getattr(optimization_result, "selected_files", [])
    excluded_files = getattr(optimization_result, "excluded_files", [])

    for priority, file_name in enumerate(selected_files):
        ...
        loaded_content.append(
            LoadedContent(
                ...
                more_available=bool(excluded_files),
...
async def _process_priority_file(
    ...
    try:
        ...
        cumulative_tokens += tokens
        more_available = priority < len(priority_order) - 1
        metadata = await loader.metadata_index.get_file_metadata(file_name) or {}
