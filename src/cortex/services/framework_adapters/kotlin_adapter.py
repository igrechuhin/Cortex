"""Kotlin Framework Adapter

Adapter for Kotlin projects using Maven or Gradle: format (Spotless/ktlint),
...
class KotlinAdapter(FrameworkAdapter):
    ...
    def type_check(self) -> CheckResult:
        ...
        try:
            result = (
                ...
                if tool == "maven"
                else self._run_gradle(["compileKotlin"])
            )
            output = result.stdout + result.stderr
    ...
    def _parse_compile_output(self, output: str) -> list[str]:
        ...
        for line in output.splitlines():
            ...
            if not s:
                continue
            if _KOTLIN_ERROR_RE.search(s) or _KOTLIN_LINE_RE.search(s):
                errors.append(s)
        return errors
