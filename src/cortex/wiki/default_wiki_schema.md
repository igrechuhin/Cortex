...
Cortex code names the two root markdown files with `WikiRootDocument` in `cortex.wiki.wiki_root_files` (`SCHEMA` → `schema.md`, `INDEX` → `index.md`) so ingest, layout, and lint stay aligned with this tree.

## Page frontmatter

Each wiki page (except the root catalog and schema files above) should start with YAML frontmatter:
