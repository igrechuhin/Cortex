...
- **MCP tool annotations (MANDATORY)**: Every `@mcp.tool()` MUST include `annotations=...` parameter using helpers from `cortex.core.mcp_annotations`: `read_only_annotations(title)` for read-only tools, `safe_write_annotations(title)` for non-destructive writes, `destructive_annotations(title)` for destructive operations, `external_annotations(title, read_only=...)` for external system interactions. See `.cortex/synapse/rules/python/python-mcp-development.mdc`.
...
1. **MCP Handler Registration**: Always use `@mcp.tool(annotations=...)` decorator with annotations; every tool MUST also have `@ensure_usage_context` and `@mcp_tool_wrapper(timeout=...)` below it (MANDATORY)
...
- Sequential Thinking: `sequentialthinking` – stepwise reasoning and planning (thought history, branches)
