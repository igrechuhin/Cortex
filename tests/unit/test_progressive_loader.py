...
class TestLoadProgressiveContextSkipsStaleIndexEntries:
    """Tests that load_progressive_context_impl skips stale index entries."""

    @pytest.mark.asyncio
    async def test_load_by_relevance_skips_stale_index_entries(
        self, temp_project_root: Path
    ) -> None:
        """Stale index entries are skipped when loading by relevance."""
        import json
        ...
        file_names = [f.get("file_name") for f in loaded_files]
        assert "existing.md" in file_names
        assert "stale.md" not in file_names
