...
class TestAccessRecording:
    ...
    async def test_updates_file_stats(self, temp_project_root: Path):
        ...
        # Assert
        stats = analyzer.access_data["file_stats"].get(file_path, {})
    ...
    async def test_updates_task_patterns(self, temp_project_root: Path):
        ...
        # Assert
        pattern = analyzer.access_data["task_patterns"].get(task_id, {})
        assert pattern.get("description") == task_description
        assert len(pattern.get("files", [])) == 2
        assert file_path in pattern.get("files", [])
        assert "other.md" in pattern.get("files", [])
...
class TestTemporalPatterns:
    ...
    async def test_calculates_average_accesses_per_day(self, temp_project_root: Path):
        ...
        # Assert
        assert "avg_accesses_per_day" in result
