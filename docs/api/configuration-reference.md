...
| `rules_folder` | string | `".cursorrules"` | — | Path to rules folder |
...
### Structure: cursor_integration

| Name | Type | Default | Description |
|------|------|---------|-------------|
| `enabled` | boolean | `true` | Whether Cursor integration is enabled |
| `symlink_location` | string | `".cursor"` | Symlink location directory |
| `symlinks.memory_bank` | boolean | `true` | Create memory bank symlink |
| `symlinks.rules` | boolean | `true` | Create rules symlink |
| `symlinks.plans` | boolean | `true` | Create plans symlink |
