...
| `.cursor/commands/` | — | **Not used in this repo** — optional local-only Cursor command files; workflows are defined under `.cortex/synapse/prompts/`. **Do not** add tracked stubs here to fix tests (duplicates MCP prompts and clutters Cursor’s command list); alignment tests skip when no `*.md` exists. `.cortex/synapse/cursor-agents/implement-code.md` syncs to `.cursor/agents/implement-code.md` and points at `docs/guides/synapse-final-report-templates.md`. |
