...
- **Synapse submodule** (shared rules and hook scripts): from repo root run `git submodule update --init --recursive` to initialize or align the gitlink. After that, the MCP server runs `git pull --ff-only origin main` inside `.cortex/synapse` once at startup (stashing and restoring local changes if present; skipped when `CORTEX_SKIP_SYNAPSE_UPDATE=1`). Submodule Git operations can fail independently of PyPI; failures there are non-fatal—check logs and run the init/pull manually if needed.
