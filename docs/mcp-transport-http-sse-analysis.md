...
|-------------------------|---------------------|---------------------|------------------------|
| **Cursor**              | ✅ Supported (default). Configure command, args, env in MCP settings. | ✅ Supported. URL form: `http(s)://host:port/sse?token=...`. | ✅ Supported via MCP Inspector at `http://localhost:8000/mcp`; Cursor can connect to URL-based servers. |
...
**Conclusion**: Cursor supports both stdio (command) and SSE/URL. Using HTTP/SSE or Streamable HTTP is **not blocked** by Cursor; users can run Cortex as a long-lived server and connect via URL instead of a shell command.
...
- **Pros**: Real concurrency; Cursor and Inspector can connect via URL; stdio remains for local/CLI and Claude Desktop.  
...
- **Rationale**:
  ...
  - Cursor supports SSE/URL; adoption is not blocked by the primary client.
...
- Cursor MCP (stdio + SSE URL): Cursor docs and community (e.g. cursor.fan, Cursor directory)
