...
└── prompts_agents.py       # Cursor/Claude agent file sync
```
...
- Calls `register_synapse_prompts()` and `sync_cursor_agents()` at import time.
...
- `get_cursor_agents_source()`: locates `.cortex/synapse/cursor-agents/` by
  walking CWD then the module-anchor tree.
...
- `sync_cursor_agents()`: called at import time; syncs to both `.cursor/agents/`
  (Cursor, no transform) and `.claude/agents/` (Claude Code, with transform).
...
prompts.py import
  ...
  └── sync_cursor_agents()                           # prompts_agents.py
        ├── sync_agents_to_target(cursor, transform=False)
        └── sync_agents_to_target(claude, transform=True)
```

## Path discovery strategy

Both `get_prompts_paths()` and `get_cursor_agents_source()` use the same two-pass
strategy:
