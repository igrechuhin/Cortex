...
**Step 0**: Call `mcp__cortex__pipeline_handoff(operation="read_task", pipeline="commit", phase="checks")` to get context from the orchestrator (coverage threshold, snapshot ref, etc.). If not found, use defaults: `coverage_threshold=0.90`.

**Step 1**: Call `mcp__cortex__start_pre_commit_job(phase="A", test_timeout=300, coverage_threshold=0.90, strict_mode=False)`.
...
**Step 2**: Poll loop — call `mcp__cortex__get_pre_commit_job_status(job_id=<job_id>)` repeatedly.
...
**Step 4** (fix path): Call `mcp__cortex__load_context(task_description="Fixing errors and quality issues for commit", token_budget=15000)`.

**Step 5**: Call `mcp__cortex__fix_quality_issues()` to auto-fix formatting, linting, and type issues.

**Step 6**: Restart job — call `mcp__cortex__start_pre_commit_job(phase="A", test_timeout=300, coverage_threshold=0.90, strict_mode=False)` again and poll (Steps 2-3).
