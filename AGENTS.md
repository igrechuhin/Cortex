...
- When editing `roadmap.md` pending bullets, avoid bare dotted Python filenames, backticked paths such as `pre_commit_foo`/`pre_commit_bar`, and the root Node manifest name written as one token; the roadmap file-reference scanner can treat those as real paths and fail `roadmap_sync`. When a bullet should point at a real plan file, end the line with `Plan: .cortex/plans/<file>.md` (session tooling parses this). `plan(operation="register")` only accepts `section` values `blockers`, `active_work`, `future`, or `pending`. Every non-archived plan under `.cortex/plans/` must be linked from `roadmap.md` or `roadmap_sync` reports it as an unlinked plan; the directory's own scaffolding (`README.md`, a `TEMPLATE*` file, `dependency-graph.md`, `index.md`) is exempt. `manage_file(operation="write", file_name="roadmap.md")` additionally enforces the memory-bank schema, which requires the sections `Future Enhancements` and `Pending plans (from .cortex/plans)` to be present — a restructured roadmap must keep those headings even when its own organising principle differs.
