...
- **Synapse submodule on startup**: From the detected project root, the server runs a best-effort, **non-fatal** `git submodule update --init --recursive` (superproject gitlink only; not `--remote`) so `.cortex/synapse` is initialized or aligned. If the submodule has local changes, they are **stashed** before the update and **restored** afterward. It is **skipped** if the root is not a git checkout or if you set `CORTEX_SKIP_SYNAPSE_UPDATE=1`. Git errors or timeouts are logged and startup continues. Manual recovery is still `git submodule update --init --recursive` from repo root (same as CI/bootstrap scripts).
